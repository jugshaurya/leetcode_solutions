...
public:
    int minimumOperations(vector<int>& nums) {
        ...
        set<int> s;
        for (int i=0;i<n;i++){
            if(i&1) oddMap[nums[i]] += 1;
            else evenMap[nums[i]] += 1;
            s.insert(nums[i]);
        }
        
        if(s.size()==1) return n/2;
        ...
        if(evenV.size()==1 and oddV.size()==1){
            if(evenV[0].second!=oddV[0].second) return 0;
            return min(evenV[0].first ,oddV[0].first);
        }
        
        if(oddV.size()==1){
            if(evenV[0].second != oddV[0].second) return n - evenV[0].first;
            return min(evenV.size()+1 + oddV[0].first - evenV[1].first , evenV.size() +1 - evenV[1].first);
            
        }
        
        if(evenV.size()==1){
            if(oddV[0].second != evenV[0].second) return n - oddV[0].first;
            return min(n/2 + evenV[0].first - oddV[1].first , n/2 - oddV[1].first);
        }
